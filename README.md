...
|`FUN_0079cfb0` | **LoadScreen_SetProgress** | Applies the load-bar progress value, optionally refreshes loading-screen hint state below 95%, and traces the new value. | no |
|`FUN_006efe20` | **LoadItemPropertyCostTables** | Loads `IPRP_COSTTABLE.2da`, allocates the cost-table array, and constructs each referenced item-property cost table. | no |
|`FUN_006e9bc0` | **LoadLoadscreens2DATable** | Constructs and loads `Loadscreens.2da` into the owning rules/configuration object. | no |
|`FUN_0079fc40` | **SelectLoadingScreenHint** | Selects and rotates normal or story hints from `loadscreenhints.2da` or `loadstoryhints.2da`, tracking the current hint per language/category. | no |
...
|`FUN_0045a030` | **Gob::LoadFromFileOrStream** | Loads a Gob game object from disk or an in-memory stream, initializes its subcomponents, and registers it with the game world. | yes |
...
|`FUN_004269F0` | **Texture_FindExisting** | Searches the global texture cache for an existing texture matching the requested name and optional variant/material parameters. | yes |
|`FUN_00426D00` | **Texture_UpdateResourceBinding** | Updates or replaces a texture resource binding, including lookup/create paths for replacement texture data. | yes |
|`FUN_00426F20` | **Texture_ReplaceAcrossUsers** | Replaces a texture across user/reference collections by finding the old texture and resolving the new one. | yes |
|`FUN_004274E0` | **Texture_AcquireAndRelease** | Small wrapper around the texture get/create path that resolves a texture and then releases or balances the reference. | yes |
|`FUN_00427520` | **Texture_GetOrCreate** | Main texture get-or-create path: checks the cache, constructs a `CAurTextureBasic` on miss, applies TXI/controller state, and registers the texture. | yes |
