...
|`FUN_00781840` | **ResourceQueue_UnpackAndTrace** | Unpacks a single resource-queue packet with full tracer setup/flush; wraps the call in the Concurrency scheduler and records timing/log data before invoking the real dispatch. | yes |
...
|`FUN_0040ee40` | **GUI_InitWidgetFromGFF** | Loads and initializes a GUI widget from its GFF layout resource. Creates a `CResGFF` object, opens the `.gui` file via the resource streamer, then parses `COLOR`/`BORDER`/`BACKGROUND`/`ALPHA`/`CONTROLS` fields and scales coordinates to screen resolution. Guards against double-init via a flag bit. Called once per panel object (e.g. `abilities_p`) during each `CSWGui*_Ctor`. Low individual cost (~150μs avg), but invoked 81 times per load across all GUI constructors. | yes |
|`FUN_0040f620` | **GUI_BindNamedWidget** | Binds a named GUI control from a loaded GFF layout into a member slot on the panel object. Calls `GUI_SearchControlsByName` (O(N) linear scan with per-iteration malloc) to find the control by tag, reads its position/size, applies screen-resolution scaling, and registers it via `ArrayAdd`. Called ~20 times per `CSWGui*_Ctor` that uses named widget lookups — the repeated linear scan makes this the suspected dominant cost inside `CSWGuiInGameAbilities_Ctor`. | yes |
